...
">
    <table cellpadding="0" cellspacing="0" style="float: right;height: 45px;">
        <tr>
            ...
            <td style="vertical-align: middle;">
                <a href="https://twitter.com/ratingwidget" data-show-screen-name="false" class="twitter-follow-button"><?php 
_e( 'Follow', WP_RW__ID );
?>
</a>
...
">
    ...
    foreach ( $multirating_options->criteria as $criterion ) {
        ...
        ?>
							<tr class="rw-rating">
								<td>
									<span class="rw-add-label"><a href="#" data-placeholder="<?php 
        _e( 'Add Label', WP_RW__ID );
        ?>
" class="<?php 
        echo  ( isset( $criterion['label'] ) && $criterion['label'] != __( 'Add Label', WP_RW__ID ) ? 'has-custom-value' : '' ) ;
        ?>
"><nobr><?php 
        echo  ( isset( $criterion['label'] ) ? $criterion['label'] : __( 'Add Label', WP_RW__ID ) ) ;
        ?>
</nobr></a></span>
								</td>
...
">[+] <?php 
        ...
        ?>
									<a class="rw-add-rating" href="#" data-upgrade-href="<?php 
        echo  $rw_fs->get_upgrade_url() ;
        ?>
" data-upgrade-text="[+] <?php 
        echo  $upgrade_label_text ;
...
">[+] <?php 
    ...
    ?>
								...
								<div class="summary-rating-option">
									<label><input type="checkbox" class="show-summary-rating" name="multi_rating[show_summary_rating]" <?php 
    checked( true, $multirating_options->show_summary_rating );
    ?>
/> <?php 
    _e( 'Show Summary Rating', WP_RW__ID );
    ?>
</label>
								</div>
							</td>
						</tr>
						<tr class="rw-summary-rating" style="<?php 
    echo  ( $multirating_options->show_summary_rating ? '' : 'display: none' ) ;
    ?>
">
                            <?php 
    $summary_label = ( isset( $multirating_options->summary_label ) && $multirating_options->summary_label != __( 'Add Label', WP_RW__ID ) ? $multirating_options->summary_label : '' );
    ?>
							<td>
								<span class="rw-add-label rw-summary-label"><a href="#" data-placeholder="<?php 
    _e( 'Add Label', WP_RW__ID );
    ?>
" class="<?php 
    ...
    ?>
"><nobr><?php 
    echo  ( !empty($summary_label) ? $summary_label : __( 'Add Label', WP_RW__ID ) ) ;
    ?>
</nobr></a></span>
							</td>
							<td colspan="2">
...
" data-read-only="true"></div>
        ...
        ?>
										<div class="rw-ui-star"<?php 
        echo  ( $default_hide_recommendations ? ' data-hide-recommendations="true" ' : '' ) ;
        ?>
 data-read-only="true"></div>
										<div class="rw-ui-nero"<?php 
        echo  ( $default_hide_recommendations ? ' data-hide-recommendations="true" ' : '' ) ;
        ?>
 data-read-only="true"></div>
									<?php 
    ...
    ?>
							...
							<input type="hidden" class="multi-rating-label" name="multi_rating[summary_label]" value="<?php 
    echo  $summary_label ;
    ?>
" />
						</tr>
...
if ( $has_multi_rating ) {
    ?>
				<h3><?php 
    _e( 'Multi-Rating Options', WP_RW__ID );
    ?>
</h3>
				<div id="multi-rating-options">
					<div>
						<label><input type="checkbox" class="hide-info-bubble" <?php 
    checked( false, $options->showInfo );
    ?>
> <?php 
    _e( 'Hide Info Bubble', WP_RW__ID );
    ?>
</label>
					</div>
					<div>
						<label><input type="checkbox" class="author-rating-readonly" <?php 
    checked( true, $options->readOnly );
    ?>
> <?php 
    _e( 'Author Rating (readOnly for visitors)', WP_RW__ID );
    ?>
</label>
					</div>
				</div>
...
esc_attr_e( 'Save Changes', WP_RW__ID );
?>
" />
            <?php 
...
if ( $rw_fs->is_not_paying() ) {
    ...
    echo  $rw_fs->get_upgrade_url() ;
    ?>
" onclick="_gaq.push(['_trackEvent', 'upgrade', 'wordpress', 'gopro_button', 1, true]); _gaq.push(['_link', this.href]); return false;" class="button-secondary gradient rw-upgrade-button" style="float: right;"><?php 
    _e( 'Upgrade Now!', WP_RW__ID );
    ?>
</a>
            <?php 
}
...
?>
            <span style="margin: 0 10px; font-size: 1em; float: right; line-height: 30px;"><b style="font-size: 24px;vertical-align: top;color: #999;">&#9829;</b> <?php 
_e( 'Like it?', WP_RW__ID );
?>
  <a href="http://wordpress.org/support/view/plugin-reviews/rating-widget?rate=5#postform" target="_blank" style="
    font-weight: bold;
"><?php 
_e( 'Support the plugin with ★ 5 Stars', WP_RW__ID );
?>
</a></span>
        </div>
    </div>
