...
<div id="rw_wp_registration" class="rw-wp-container rw-dir-ltr wrap">
    <h2><?php 
_e( 'You are just 30 sec away from boosting your blog with ratings...', WP_RW__ID );
?>
</h2>
    <div id="iframe"></div>
    ...
    (function($){
        $(function(){
          ...
          var if_height,
                domain = '<?php 
echo  ( WP_RW__LOCALHOST_SCRIPTS ? WP_RW__ADDRESS : WP_RW__SECURE_ADDRESS ) ;
?>
',
            ...
            iframe = $('<iframe " src="' + src + '" width="100%" height="650" scrolling="no" frameborder="0" style="background: transparent;"><\/iframe>')
                .load(function(){
                      ...
                      $.postMessage(JSON.stringify({
                          first: '<?php 
echo  addslashes( $current_user->user_firstname ) ;
?>
',
                          last: '<?php 
echo  addslashes( $current_user->user_lastname ) ;
?>
',
                          email: '<?php 
echo  addslashes( $current_user->user_email ) ;
?>
',
                          address: address,
                          title: '<?php 
echo  addslashes( get_option( 'blogname', '' ) ) ;
?>
'
