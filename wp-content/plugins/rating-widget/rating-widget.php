...
//						add_action( 'admin_head', array( &$this, "GoogleAnalytics" ) );
			...
			function init_toprated_shortcode_settings() {
				...
				$max_item_count = $this->_rw_is_running() ? 50 : 11;
				$max_items = array();

				for ($count = 1; $count <= $max_item_count; $count++) {
					if ($count === $max_item_count && !$this->_rw_is_running()) {
...
--------------------------------------------------------------------------------------------*/
    ...
    ---------------------------------------------------------------------------------------------------------------*/
			...
			function _rw_setup_extension()
			{
				return (!$this->_rw_potential() && !$this->_rw_check_if_done());
			}

			function _rw_check_if_done()
			{
			...
			function _rw_is_running()
			{
				
				if ($this->_rw_potential())
					return true;
			...
			function SetupMenuItems()
			{
				...
				/*if ($this->_rw_is_running() && !$this->_rw_potential())
					// Boosting.
					$submenu[] = array(
    ...
    ---------------------------------------------------------------------------------------------------------------*/
            ...
            ---------------------------------------------------------------------------------------------------------------*/
					if (isset($_POST['multi_rating'])) {
						...
						if (!$this->_rw_is_running() && count($multi_rating['criteria']) > 3) {
    ...
    ---------------------------------------------------------------------------------------------------------------*/
			...
			private function GetRatingHtml($pUrid, $pElementClass, $pAddSchema = false, $pTitle = "", $pPermalink = '', $pOptions = array())
			{
				...
				if (true === $pAddSchema && 'front-post' !== $pElementClass && $this->_rw_is_running() && !isset($_REQUEST['schema_test']) )
				{
					$rich_snippet_settings = $this->get_rich_snippet_settings();
...
//            "forum-topic" => array("options" => WP_RW__ACTIVITY_FORUM_TOPICS_OPTIONS),
				...
				if ($attach_js || $this->_TOP_RATED_WIDGET_LOADED)
				{
                    ...
                    ?>,
									source: "wordpress",
									options: {
									<?php if ($this->_rw_is_running() && defined('ICL_LANGUAGE_CODE') && isset($this->languages[ICL_LANGUAGE_CODE])) : ?>
